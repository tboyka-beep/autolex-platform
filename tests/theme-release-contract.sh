...
grep -Fq "RELEASE_TARGET: \${{ github.event.inputs.target || 'staging' }}" "$workflow"
grep -Fq "environment: \${{ github.event.inputs.target || 'staging' }}" "$workflow"
grep -Fq 'if [ "$RELEASE_TARGET" = production ]' "$workflow"
