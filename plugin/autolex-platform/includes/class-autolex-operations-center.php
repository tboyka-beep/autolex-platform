...
final class Autolex_Operations_Center
{
        ...
        <div class="wrap autolex-operations">
            ...
            <section class="alxo-status alxo-status--<?php echo esc_attr($health); ?>">
                ...
                <p>
                    <?php
                    printf(
                        /* translators: 1: completed targets, 2: all targets, 3: progress percent. */
                ...
                <p class="alxo-meta">
                    <?php
                    printf(
                        /* translators: 1: next scheduled run, 2: last attempt. */
    ...
    public function handle_action()
    {
        ...
        switch ($operation) {
            ...
            case 'recover_stale':
                ...
                wp_schedule_single_event(time() + 5, 'autolex_eea_sync_batch');
                $message = sprintf(
                    /* translators: %d: recovered task count. */
                    __('%d beragadt feladat került biztonságosan újrapróbálható állapotba.', 'autolex-platform'),
                    $recovered
                );
                break;
            ...
            case 'retry_failed':
                ...
                wp_schedule_single_event(time() + 5, 'autolex_eea_sync_batch');
                $message = sprintf(
                    /* translators: %d: retried task count. */
                    __('%d hibás forrásfeladat került vissza a feldolgozási sorba.', 'autolex-platform'),
                    $retried
                );
                break;
        ...
        set_transient(
            'autolex_operations_notice_' . get_current_user_id(),
        ...
        $count = (int) $wpdb->query(
            $wpdb->prepare(
                ...
                SET status = 'retry', locked_at = NULL, next_run_at = %s,
                    last_error = 'Recovered manually from a stale background lock.', updated_at = %s
                WHERE status = 'running' AND (locked_at IS NULL OR locked_at < %s)",
                $now,
                $now,
                $stale
            )
        ...
        $count = (int) $wpdb->query(
            $wpdb->prepare(
                ...
                SET status = 'retry', attempts = 0, locked_at = NULL,
                    next_run_at = %s, completed_at = NULL, updated_at = %s
                WHERE status = 'failed'",
                $now,
                $now
            )
    ...
    private function get_failed_tasks()
    {
        ...
        $rows  = $wpdb->get_results(
            "SELECT target_type, make, commercial_name, source_year, attempts, last_error
            FROM {$table}
            WHERE status = 'failed'
    ...
    {
        ?>
        <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>"<?php echo $confirmation ? ' onsubmit="return confirm(' . esc_attr(wp_json_encode($confirmation)) . ');"' : ''; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
